...
):  # Tree is just a root node - no SNV can be added
    empty_model = SNVModel.create_empty_model(ctxt)
    cells: CellsData = generator.gen.sample_cells_data(
        10, 100, empty_model, ctxt=ctxt, random_attachment=True
    )[0]
    inferred_snvs = MLSolver(
        cd=cells,
        tree=EventTreeWithCounts(
            tree=empty_model.tree, node_to_cn_profile=empty_model.node_to_cn_profile
        ),
        p=Parameters(
            e=ctxt.sequencing_error(),
            m=ctxt.per_allele_coverage(),
            q=ctxt.read_success_prob(),
        ),
...
def test_model_without_snv(ctxt: SNVGeneratorContext):  # Tree has no SNVs
    ...
    empty_model.node_to_cn_profile[(0, 10)] = empty_model.node_to_cn_profile[
        EventTreeRoot
    ]
    empty_model.node_to_cn_profile[(5, 15)] = empty_model.node_to_cn_profile[
        EventTreeRoot
    ]
    empty_model.node_to_altered_counts_profile[
        (0, 10)
    ] = empty_model.node_to_altered_counts_profile[EventTreeRoot]
    empty_model.node_to_altered_counts_profile[
        (5, 15)
    ] = empty_model.node_to_altered_counts_profile[EventTreeRoot]

    cells: CellsData = generator.gen.sample_cells_data(
        50, 100, empty_model, ctxt=ctxt, random_attachment=False
    )[0]
    inferred_snvs = MLSolver(
        cd=cells,
        tree=EventTreeWithCounts(
...
def test_model_with_snvs(
    ctxt: SNVGeneratorContext,
):  # Tree has SNVs, sample size is sufficient - everything should be inferred correctly
    model = generator.gen.SNVModelGenerator(ctxt=ctxt).generate_model(4)
    cells: CellsData = generator.gen.sample_cells_data(
        50, 100, model, ctxt=ctxt, random_attachment=False
    )[0]
    inferred_snvs = MLSolver(
        cd=cells,
        tree=EventTreeWithCounts(
            tree=model.tree, node_to_cn_profile=model.node_to_cn_profile
        ),
        p=Parameters(
            e=ctxt.sequencing_error(),
            m=ctxt.per_allele_coverage(),
