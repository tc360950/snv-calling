...
class AncestryRecall(Statistic):
    def get_scores(self, real_tree: EventTree, inferred_tree: EventTree) -> Dict[str, float]:
        real_node_snvs = {(n, snv) for n in real_tree.cn_event_tree.nodes for snv in real_tree.node_to_snvs.get(n, set())}
        ...
        for n, snv in real_node_snvs:
            for n2, snv2 in real_node_snvs:
                if n == n2 and snv == snv2:
                    continue
                if n in nx.ancestors(real_tree.cn_event_tree, n2) or n == n2:
                    real_ancestry_pairs += 1
                    if self.__has_ancestor_descendant_pair(inferred_snv_to_nodes.get(snv, set()), inferred_snv_to_nodes.get(snv2, set()), inferred_tree.cn_event_tree):
                        found_ancestry_pairs += 1
                elif n2 in nx.ancestors(real_tree.cn_event_tree, n) or n == n2:
                    real_ancestry_pairs += 1
                    if self.__has_ancestor_descendant_pair(inferred_snv_to_nodes.get(snv2, set()), inferred_snv_to_nodes.get(snv, set()), inferred_tree.cn_event_tree):
                        found_ancestry_pairs += 1
        return {
