...
from solver.input import create_cn_matrix, EventTreeWithCounts
...
class NewtonRhapsonEstimator:
    ...
    def calculate_jacobian(self, p: Parameters) -> np.ndarray:
        ...
        f_1_e = 0.0 if np.sum(S_0 * S_0 * B_inv_2) == 0.0 else -math.exp(math.log(np.sum(S_0 * S_0 * B_inv_2)) + 2.0 * log_coef)
        ...
        f_1_q = -math.log(1 - p.q) * math.exp(math.log(np.sum(S_0)) - 2.0 * math.log(p.q)) \
                ...
                + 0.0 if np.sum(S_0 * A * B_inv_2) == 0.0 else math.exp(math.log(np.sum(S_0 * A * B_inv_2)) - 2.0 * math.log(p.q) + log_coef) + \
                - 0.0 if np.sum(S_0 * B_inv) == 0.0 else math.exp(math.log(np.sum(S_0 * B_inv)) - 2.0 * math.log(p.q))
