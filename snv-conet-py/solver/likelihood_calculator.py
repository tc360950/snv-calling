...
class LikelihoodCalculator:
    ...
    def get_likelihood_refresher(self):
        ...
        self.__calculate_d_likelihood(range(0, self.context.get_number_of_bins()), d_log_lik)
        self.__calculate_b_likelihood(range(0, self.context.get_number_of_bins()), b_log_lik)

        def __calculate(bin: Optional[int]) -> float:
            if bin is None:
                return logsumexp(d_log_lik + b_log_lik)
            self.__calculate_b_likelihood([bin], b_log_lik)
            return logsumexp(d_log_lik[:, bin] + b_log_lik[:, bin])

        return __calculate

    def __calculate_b_likelihood(self, bins: Iterable[int], b_lik: np.ndarray) -> None:
        def gt_log_lik(g: Genotype, b: int, d: int) -> float:
            if g.altered == 0:
        ...
        for node in self.model.event_tree.cn_event_tree.nodes:
            ...
            for cell in [c for c in range(0, b_lik.shape[0]) if self.cell_data.attachment[c] == node]:
                for bin in bins:
                    ...
                    b_lik[cell, bin] = \
                        logsumexp([-np.log(len(genotypes)) + gt_log_lik(g, self.cell_data.b[cell, bin],
    ...
    def __calculate_d_likelihood(self, bins: Iterable[int], d_lik: np.ndarray) -> None:
        def __get_bin_log_lik(d: int, cn: int, cluster_size: int) -> float:
            mean = cluster_size * (self.context.get_d_sampling_error() if cn == 0 else cn * self.context.get_per_allele_coverage())
            num_failures = int(
    ...
    def __get_possible_genotypes_supplier(self, node: CNEvent) -> Callable[[int], List[Genotype]]:
        bit_map = self.model.event_tree.mark_bins_with_cn_change_after_alteration(node, np.zeros((self.context.get_number_of_bins())))
        bins_with_snvs = self.model.event_tree.mark_bins_wth_snv(node, np.zeros((self.context.get_number_of_bins())))

        def __get_possible_genotypes_for_bin(bin: int) -> List[Genotype]:
            cn = self.model.node_to_cn_profile[node][bin]
            if bins_with_snvs[bin] == 0.0:
