...
class LikelihoodCalculator:
    ...
    def __calculate_b_likelihood(self, bins: Iterable[int], b_lik: np.ndarray) -> None:
        def l(g: Genotype, b: int, d: int) -> float:
            if g.altered == 0:
                return scipy.stats.binom.logpmf(b, d, self.p.e)
    ...
    def __calculate_d_likelihood(self, bins: Iterable[int], d_lik: np.ndarray) -> None:
        def __get_bin_log_lik(d: int, cn: int, cluster_size: int) -> float:
            ...
            num_failures = int(math.exp(math.log(1.0 - self.p.q) + math.log(mean) - math.log(self.p.q)))
            if num_failures == 0:
                return 0.0
            return scipy.stats.nbinom.logpmf(d, num_failures, 1 - self.p.q)
    ...
    def __get_possible_genotypes_supplier(self, node: CNEvent) -> Callable[[int], List[Genotype]]:
        bit_map = self.tree.tree.mark_bins_with_cn_change_after_alteration(node, np.zeros((self.cell_data.d.shape[1])))
        bins_with_snvs = self.tree.tree.mark_bins_with_snv(node, np.zeros((self.cell_data.d.shape[1])))

        def __get_possible_genotypes_for_bin(bin: int) -> List[Genotype]:
            cn = self.tree.node_to_cn_profile[node][bin]
