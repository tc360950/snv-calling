...
from core.model_data import Parameters, CellsData, EventTreeWithCounts
from core.types import CNEvent, Bin
...
class LikelihoodCalculator:
    ...
    def __calculate_b_likelihood(self, bins: Iterable[int], b_lik: np.ndarray) -> None:
        def l(g: Genotype, b: int, d: int) -> float:
            return binom.logpmf(b, d, self.p.e) if g.altered == 0 else binom.logpmf(b, d, g.altered / g.cn)
        ...
        for node in self.tree.tree.cn_event_tree.nodes:
            ...
            for cell in self.cell_data.get_cells_attached_to_node(node):
                for bin in bins:
                    ...
                    b_lik[cell, bin] = \
                        logsumexp([-np.log(len(genotypes)) + l(g, self.cell_data.b[cell, bin], self.cell_data.d[cell, bin]) for g in genotypes])
    ...
    def __calculate_d_likelihood(self, bins: Iterable[int], d_lik: np.ndarray) -> None:
        def __get_bin_log_lik(d: int, cn: int, cluster_size: int) -> float:
            ...
            return 0.0 if num_failures == 0 else nbinom.logpmf(d, num_failures, 1 - self.p.q)

        for cell in self.cell_data.get_cells():
            for bin in bins:
                d_lik[cell, bin] = __get_bin_log_lik(d=self.cell_data.d[cell, bin],
                                                     cn=self.tree.node_to_cn_profile[self.cell_data.attachment[cell]][bin],
                                                     cluster_size=self.cell_data.cell_cluster_sizes[cell])

    def __get_possible_genotypes_supplier(self, node: CNEvent) -> Callable[[Bin], List[Genotype]]:
        bit_map = self.tree.tree.mark_bins_with_cn_change_after_alteration(node, np.full((self.cell_data.d.shape[1]), False))
        bins_with_snvs = self.tree.tree.mark_bins_with_snv(node, np.full((self.cell_data.d.shape[1]), False))
