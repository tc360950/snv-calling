...
import numpy as np
from core.model_data import CellsData, EventTreeWithCounts, Parameters, create_cn_matrix
...
class MMEstimator:
    ...
    def __sum_copy_numbers(cells: CellsData, CN_profiles: np.ndarray) -> int:
        return sum(
            [
                cells.cell_cluster_sizes[c]
                * np.sum(
                    CN_profiles[
                        c,
                    ]
                )
                for c in range(0, cells.d.shape[0])
            ]
        )

    @staticmethod
    def __count_bin_cell_pairs_with_zero_copy(
        cells: CellsData, CN_profiles: np.ndarray
    ) -> int:
        return sum(
            [
                cells.cell_cluster_sizes[c]
...
class NewtonRhapsonEstimator:
    ...
    def __create_S_matrix(self) -> np.ndarray:
        ...
        for cell in range(0, self.cells.d.shape[0]):
            result[
                cell,
            ] = self.cells.cell_cluster_sizes[cell]
        return result
    ...
    def __create_alpha_matrix(self, params: Parameters) -> np.ndarray:
        ...
        for cell in range(0, self.cells.d.shape[0]):
            b[cell,] *= (
                self.cells.cell_cluster_sizes[cell] * params.m
            )
        ...
        for cell in range(0, result.shape[0]):
            for bin in range(0, result.shape[1]):
                result[cell, bin] = func(
                    coef * alpha[cell, bin], self.cells.d[cell, bin]
                )
        return result
