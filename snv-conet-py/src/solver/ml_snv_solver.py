from typing import Tuple, Set, Optional, List, Callable
...
class MLSolver:
    ...
    def insert_snv_events(self, snv_event_candidates: List[SNVEvent]) -> EventTree:
        ...
        logger.info(f"Starting SNV events inference, log-likelihood of tree with 0 SNVs: {log_likelihood}.")
        ...
        logger.info(f"Finished SNV events inference. Log-likelihood {calculator_refresh(None)}.")
        return self.tree.tree

    def __choose_best_svn_location(self, snv: SNVEvent, calculator_refresh: Callable[[SNVEvent], float]) -> Tuple[Optional[CNEvent], float]:
        logger.info(f"Choosing best location for SNV {snv}. Likelihood without SNV: {calculator_refresh(snv)}.")
        location_to_lik = {}
        ...
        best_node, best_lik = next(iter([(n, l) for n, l in location_to_lik.items() if l == max(location_to_lik.values())]), (None, 0.0))
        logger.info(f"Best location is node {best_node} with log-likelihood {best_lik}.")
