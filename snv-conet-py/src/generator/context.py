...
class SNVGeneratorContext:
    ...
    def sample_cn_change(self, event: CNEvent, parent_cn_profile: np.ndarray, overlap_bit_map: np.ndarray) -> int:
        """
            ...
            overlap_bit_map - boolean bitmap, @overlap_bit_map[bin] == True means that @bin is subject to some
            event in the subtree of @event (and hence no full CN deleteion is possible in that bin)
        """
        max_amplification = min(2, self.max_cn() - np.max(parent_cn_profile[range(event[0], event[1])]))
        if np.all(overlap_bit_map == False):  # No bin is present in child nodes - we can do full deletion
        ...
        return \
            sample_conditionally_with_replacement(1, lambda: random.randint(max_possible_deletion, max_amplification),
                                                  lambda x: x != 0)[0]
    ...
    def get_number_of_alterations(self, cn_before: int, cn_after: int, parent_altered_counts: int) -> int:
        """
        How many altered copies shall be present in a bin, which had @cn_before CN and @parent_altered_counts altered copies
        ...
        if cn_after > cn_before:
            return parent_altered_counts + (cn_after - cn_before) \
                if random.randint(1, cn_before) <= parent_altered_counts else parent_altered_counts
        else:
