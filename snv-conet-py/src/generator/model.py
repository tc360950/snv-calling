...
from generator.corrected_counts_generator import CountsGenerator, CNSampler
from generator.event_tree_generator import EventTree
...
class CellDataSampler:
    ...
    def generate_cell_in_node(self, node: CNEvent, model: SNVModel) -> CellData:
        ...
        for bin in range(0, d.shape[0]):
            ...
            b[bin] = self.__sample_altered_reads_in_bin(model.node_to_altered_counts_profile[node][bin], d[bin],
                                                        model.node_to_cn_profile[node][bin])

        return CellData(d=d, b=b, attachment=node, corrected_counts=cc)

    def generate_cell_attachment(self, model: SNVModel) -> Generator[CNEvent, CNEvent, Any]:
    ...
    def __create_node_attachment_probabilities(self, model: SNVModel) -> Dict[CNEvent, float]:
        """
