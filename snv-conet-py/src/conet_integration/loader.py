...
class CONETLoader:
    ...
    def __init__(self, dir: str):
        ...
        self.inferred_counts = np.loadtxt(f"{dir}{CONETLoader.COUNTS_FILENAME}").astype(int)
    ...
    def get_event_tree(self) -> EventTreeWithCounts:
        return EventTreeWithCounts(
            ...
            node_to_cn_profile=dict(
                [(self.attachment[cell], self.inferred_counts[cell, :]) for cell in range(0, len(self.attachment))]))
    ...
    def __convert_conet_node(self, node: Tuple[int, int]) -> CNEvent:
        if node != EventTreeRoot:
            return self.__breakpoint_candidate_idx_to_bin(node[0]), self.__breakpoint_candidate_idx_to_bin(node[1])
