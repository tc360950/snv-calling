...
class EventTree:
    ...
    def __copy__(self):
        return EventTree(cn_event_tree=self.cn_event_tree.copy(), node_to_snvs=self.node_to_snvs.copy())
    ...
    def get_breakpoint_loci(self) -> List[Bin]:
        return sorted(list(set([x[0] for x in self.cn_event_tree.nodes] + [x[1] for x in self.cn_event_tree.nodes])))
    ...
    def snv_can_be_added_in_node(self, snv: SNVEvent, node: CNEvent) -> bool:
        ...
        """
        return snv not in self.__gather_snvs_in_subtree(node) and snv not in self.__gather_snvs_on_path_to_root(node)

    def mark_bins_in_descendant_events(self, node: CNEvent, bin_bit_map: np.ndarray) -> np.ndarray:
        """
        ...
        def __mark_overlapping_bins(event: CNEvent) -> None:
            bin_bit_map[[b for b in range(event[0], event[1]) if node[0] <= b < node[1]]] = True
    ...
    def mark_bins_with_cn_change_after_alteration(self, node: CNEvent, bin_bit_map: np.ndarray) -> np.ndarray:
        ...
        for n in self.__get_path_from_root(node):
            ...
            bit_map[[b for b in range(n[0], n[1]) if bit_map[b] == BIN_IN_SNV]] = CN_ALTERATION_AFTER_SNV
    ...
    def __get_path_from_root(self, node: CNEvent) -> List[CNEvent]:
        return [] if node == EventTreeRoot else\
            next(nx.all_simple_paths(source=EventTreeRoot, target=node, G=self.cn_event_tree))
