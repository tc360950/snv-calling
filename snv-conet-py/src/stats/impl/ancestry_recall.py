...
class AncestryRecall(Statistic):
    def get_scores(
        self, real_tree: EventTree, inferred_tree: EventTree
    ) -> Dict[str, float]:
        real_node_snvs = {
            (n, snv)
            for n in real_tree.cn_event_tree.nodes
            for snv in real_tree.node_to_snvs.get(n, set())
        }
        inferred_node_snvs = {
            (n, snv)
            for n in inferred_tree.cn_event_tree.nodes
            for snv in inferred_tree.node_to_snvs.get(n, set())
        }
        ...
        for n, snv in real_node_snvs:
            for n2, snv2 in real_node_snvs:
                ...
                if n in nx.ancestors(real_tree.cn_event_tree, n2):
                    ...
                    if self.__has_ancestor_descendant_pair(
        ...
        return {
            "AncestryRecall": 0.0
            if real_ancestry_pairs == 0
            else found_ancestry_pairs / real_ancestry_pairs
        }

    def __has_ancestor_descendant_pair(
        self, nodes1: Set[CNEvent], nodes2: Set[CNEvent], tree: nx.DiGraph
    ) -> bool:
        for n1 in nodes1:
            for n2 in nodes2:
