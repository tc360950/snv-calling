...
class SNVModel:
    tree: EventTree
    node_to_cn_profile: Dict[Node, np.ndarray]
    node_to_altered_counts_profile: Dict[Node, np.ndarray]
    ...
    def create_empty_model(cls, ctxt: SNVGeneratorContext) -> "SNVModel":
        ...
        return cls(
            tree=EventTree(event_tree=tree),
            node_to_cn_profile={
...
class CellDataSampler:
    ...
    def generate_cell_in_node(self, node: Node, model: SNVModel) -> CellData:
    ...
    def __create_node_attachment_probabilities(
        ...
        """
        depths = nx.shortest_path_length(model.tree.event_tree, source=EventTreeRoot)
