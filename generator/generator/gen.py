...
from generator.core.types import CNEvent, EventTreeRoot, Node, SNVEvent
...
def _deterministic_attachment(model: SNVModel) -> Generator[CNEvent, CNEvent, None]:
    nodes = [n for n in model.tree.event_tree.nodes]
    while True:
        for n in nodes:
            yield n
...
class SNVModelGenerator:
    ...
    def generate_model(self, tree_size: int) -> SNVModel:
        ...
        apply_to_nodes_in_order(
            root=EventTreeRoot,
            tree=self.model.tree.event_tree,
    ...
    def __generate_node_altered_counts_profile(self, node: Node) -> None:
