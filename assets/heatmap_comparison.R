...
# TODO posortowac to wsyztsko po chromosmach 
...
NEUTRAL_CN <- 2
path <- "./"
inferred_counts <- t(read.table("inferred_counts"))

real_counts <- read.table("./real_data/cn_matrix.csv", sep=",", header=T)

cc <- read.table("./real_data/cc", sep=",", header=T)

chr_num <- 1 
chrm_ends_idx <- c()
for (i in 2:dim(real_counts)[1]) {
  if (real_counts$chr[i] != real_counts$chr[i-1]) {
    chrm_ends_idx <- c(chrm_ends_idx, chr_num + i-1)
    chr_num <- chr_num + 1
  }
}

chrm_ends_idx <- c(chrm_ends_idx, dim(inferred_counts)[1])


inferred_counts <- inferred_counts[-chrm_ends_idx, ]
cc <- cc[-chrm_ends_idx, ]

prepare_CNs_heatmap_input <- function(CNs_data, main_CN) {
  CNs_heatmap <- as.matrix(transpose(CNs_data[,-c(1,2,3,4)]))
  
  rownames(CNs_heatmap) <- colnames(CNs_data)[-c(1,2,3,4)]
...
CNs_heatmap_input <- prepare_CNs_heatmap_input(CNs_data = real_counts, main_CN = NEUTRAL_CN) 
...
######### Inferred counts
inf <- real_counts
inf[, -c(1,2,3,4)] <- inferred_counts
CNs_heatmap_input <- prepare_CNs_heatmap_input(CNs_data = inf, main_CN = NEUTRAL_CN) 
CNs_heatmap <- CNs_heatmap_input[[1]]

tiff(paste0(path, "/heatmap_CNs_inferred", ".tiff"), units="in", width=13, height=8, res=image_qual)
heatmap3(CNs_heatmap, col=CNs_heatmap_input[[5]], breaks=CNs_heatmap_input[[4]], Colv = NA, Rowv = as.dendrogram(row_hc), scale = "none", labCol = CNs_heatmap_input[[2]], cexCol = 1.5, lasCol=1, add.expr = abline(v=CNs_heatmap_input[[3]], col="black", lwd=0.2, lty=3),
         legendfun=function() showLegend(legend=c(0:(2*NEUTRAL_CN)),col=CNs_heatmap_input[[5]]))
dev.off()